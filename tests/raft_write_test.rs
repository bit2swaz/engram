...
async fn single_node_raft_write_commits_to_state_machine() {
    ...
    let consolidated = Arc::new(engram::consolidation::store::InMemoryConsolidatedStore::default())
        as Arc<dyn engram::consolidation::store::ConsolidatedMemoryStore>;
    let metrics = Arc::new(engram::metrics::AppMetrics::new().unwrap());
